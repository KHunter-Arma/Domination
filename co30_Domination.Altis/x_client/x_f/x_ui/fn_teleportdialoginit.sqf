...

{
	...
	if (!isNull _mrs) then {
		private _lbcolor = call {
			...
			if ((surfaceIsWater (getPosWorld _mrs)) && {!(_mrs iskindof "Ship")}) exitWith {_logtxt = format [localize "STR_DOM_MISSIONSTRING_594", _x # 1, _logtxt]; __COLRED};
			if ((_mrs iskindof "Ship") && {underwater _mrs}) exitWith {_logtxt = format [localize "STR_DOM_MISSIONSTRING_594", _x # 1, _logtxt]; __COLRED};
